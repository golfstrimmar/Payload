...
const ClockUhr: React.FC<InputProps> = ({ value, onChange, disabled }) => {
  ...
  const [hour, setHour] = useState<string>(value ? value.split(':')[0].padStart(2, '0') : '00')
      ...
      <div className={styles['clock-picker']}>
        <div className={styles['clock-face']}>
          <div className={styles['clock-face-display']}>
            <div className={`${styles['time-display']} ${styles['time-display-hour']}`}>
              {time ? time.split(':')[0] : '00'}
            </div>
            <span>:</span>
            <div className={`${styles['time-display']}`}>{time ? time.split(':')[1] : '00'}</div>
          </div>
          {hours.map((h) => {
            ...
            return (
              <div
                ...
                style={{
                  left: `calc(50% + ${Math.round(x * 1.2)}px)`,
                  top: `calc(50% + ${Math.round(y * 1.2)}px)`,
        ...
        <div className={styles['minutes-face']}>
          <div className={styles['clock-face-display']}>
            <div className={`${styles['time-display']}`}>{time ? time.split(':')[0] : '00'}</div>
            <span>:</span>
            <div className={`${styles['time-display']} ${styles['time-display-hour']}`}>
            ...
            return (
              <div
                ...
                className={`${styles.minute} ${
                  m.toString().padStart(2, '0') === minute ? styles.selected : ''
                }`}
                style={{

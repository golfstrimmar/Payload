...
const Button: React.FC<ButtonProps> = ({
  ...
  const [ripples, setRipples] = useState<{ x: number; mValue: number; y: number; key: number }[]>(
    [],
  )
  ...
  const handleButtonClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (closestButton) {
      const mValue = Math.max(closestButton.clientWidth, closestButton.clientHeight)
